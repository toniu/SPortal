  ...
  private async onSave(): Promise<void> {
    ...
    try {
      for (const user of this.attendees) {
        const userInfo: any = await UserEventService.getUserByLoginName(user.id, this.props.siteUrl);
      ...
      switch (panelMode) {
        case IPanelModelEnum.edit:
          await UserEventService.updateEvent(eventData, this.props.siteUrl, this.props.listId);
          break;
        case IPanelModelEnum.add:
          await UserEventService.addEvent(eventData, this.props.siteUrl, this.props.listId);
          break;
        default:
          break;
      }
    ...
    if (this.props.panelMode === IPanelModelEnum.edit && event) {
      ...
      if (attendees && attendees.length > 0) {
        for (const userId of attendees) {
          const user: any = await UserEventService.getUserById(userId, this.props.siteUrl);
          if (user) {
  ...
  private async confirmDelete(ev: React.MouseEvent<HTMLDivElement>): Promise<void>  {
    ...
    try {
      ...
      switch (this.props.panelMode) {
        case IPanelModelEnum.edit:
          await UserEventService.deleteEvent(this.state.eventData, this.props.siteUrl, this.props.listId, this.state.recurrenceSeriesEdited);
          break;
        default:
          break;
      }
