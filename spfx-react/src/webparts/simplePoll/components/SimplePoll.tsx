...
import { IQuestionDetails, IResponseDetails, IPollAnalyticsInfo } from '../../../Models/index';
...
export default class SimplePoll extends React.Component<ISimplePollProps, ISimplePollState> {
  ...
  private _getSelectedKey = (): string => {
    ...
    if (this.state.UserResponse && this.state.UserResponse.length > 0) {
      let userResponses = this.state.UserResponse;
      let userRes = this.getUserResponse(userResponses);
      if (userRes.length > 0) {
  ...
  private _submitVote = async () => {
    ...
    let curUserRes = this.getUserResponse(this.state.UserResponse);
  ...
  private bindResponseAnalytics = () => {
    ...
    if (tmpUserResponse && tmpUserResponse.length > 0) {
      ...
      let pollAnalytics: IPollAnalyticsInfo;
      pollAnalytics = {
